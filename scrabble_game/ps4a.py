...
def getWordScore(word, n):
    ...
    if len(word) == n:
        wordScore = (len(word)*sum)+50
    else:
        wordScore = (len(word)*sum)
    return wordScore
...
def playGame(wordList):
    ...
    """
    while playHand:
        dicission = input ("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
        if dicission == 'n':
            x= dealHand(HAND_SIZE)
            playHand(x,wordList,HAND_SIZE)
        elif dicission == 'r':
            try:
                playHand(x,wordList,HAND_SIZE)
            except:
